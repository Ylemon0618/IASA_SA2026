...
class FFTResearchPipeline:
    ...
    def run(self):
        ...
        print(
            f"\n{Fore.GREEN}=== FFT Pipeline Finished Successfully ==={Style.RESET_ALL}"
        )
        print(
            f"{Fore.CYAN}[SYSTEM] Total Pipeline Execution Time: "
            f"{elapsed_time:.2f} seconds ({elapsed_time / 60:.2f} minutes / {elapsed_time / 3600:.2f} hours){Style.RESET_ALL}"
        )
